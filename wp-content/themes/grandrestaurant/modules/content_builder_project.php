...
function portfolio_content_create_meta_box() {
	...
	if ( function_exists('add_meta_box') && isset($page_postmetas) && count($page_postmetas) > 0 ) {  
		add_meta_box( 'content_metabox', 'Content Builder Option', 'portfolio_content_new_meta_box', 'projects', 'normal', 'high' );
	}
...
?>
	...
	<?php
		foreach($ppb_shortcodes as $key => $ppb_shortcode)		
		{
			...
			{
	?>
	<li data-module="<?php echo esc_attr($key); ?>" data-title="<?php echo esc_attr($ppb_shortcode['title']); ?>" data-type="module"><img src="<?php echo get_template_directory_uri(); ?>/functions/images/builder/<?php echo esc_attr($ppb_shortcode['icon']); ?>" alt="" title="<?php echo esc_attr($ppb_shortcode['title']); ?>" class="builder_thumb"/>
	...
	?>
			<li id="<?php echo esc_attr($ppb_form_item); ?>" class="ui-state-default <?php echo esc_attr($ppb_form_item_size[0]); ?> <?php echo esc_attr($ppb_form_item_data_obj->shortcode); ?>" data-current-size="<?php echo esc_attr($ppb_form_item_size[0]); ?>">
				...
				<a data-rel="<?php echo esc_attr($ppb_form_item); ?>" href="<?php echo admin_url('admin-ajax.php?action=pp_ppb&ppb_post_type=portfolios&shortcode='.$ppb_form_item_data_obj->shortcode.'&rel='.$ppb_form_item.'&width=800&height=900'); ?>" class="ppb_edit"></a>
